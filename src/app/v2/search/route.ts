...
export async function POST(request: NextRequest) {
  ...
  const { results, engines: usedEngines, resolvedLang } = await searchEngines(effectiveQuery, { limit, engines, lang: requestedLang });
  ...
  // merge the scraped content into the search result items.
  const scrapeOpts: Record<string, any> | undefined = body.scrapeOptions;
  let enriched: typeof results;
  if (scrapeOpts && typeof scrapeOpts === 'object') {
    const formats: string[] = Array.isArray(scrapeOpts.formats) ? scrapeOpts.formats : ['markdown'];
    enriched = await mapWithConcurrency(results, 3, async (item) => {
      try {
